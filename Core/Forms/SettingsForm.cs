...
namespace SewingProduction.form
{
	public partial class SettingsForm : CustomForm, IDataUpdatableForm
	{
		...
		{
			List<PrinterItem> result = new List<PrinterItem>();
			...
			gridControlPrinters.DataSource = result;

			string defaultPrinter = GetDefaultPrinter();
			comboBoxPrinters.SelectedIndex = comboBoxPrinters.Items.IndexOf(defaultPrinter);
		}
		...
		private void cButSaveDefoltPrinter_Click(object sender, EventArgs e)
		{
			...
			bool result = SetDefaultPrinter(printerName);
			if (!result)
			{
				MessageBox.Show("Не удалось изменить принтер по умолчанию.");
				return;
			}

			MessageBox.Show($"Принтер по умолчанию изменен:\n{printerName}");
			LoadPrinters();
		}
		#endregion
		#region принтеры 
