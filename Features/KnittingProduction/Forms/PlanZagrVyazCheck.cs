...
namespace SewingProduction.Features.KnittingProduction.Forms
{
    public partial class PlanZagrVyazCheck : CustomForm, IThemeable
    {
        ...
        private async Task InitializeBindingsAsync()
        {
            try
            {
                _sprMonthBindingSource = new BindingSource { DataSource = new BindingList<SprMonth>() };
                #region описание comboBox "Список месяцев"
        ...
        private async Task LoadSprMonthDataAsync()
        {
            ...
            try
            {
                ...
                await this.InvokeAsync(() =>
                {
                    ...
                    _sprMonthBindingSource.DataSource =
                        new BindingList<SprMonth>();
                });
                ...
                if (bs.Count == 0)
                {
                    await _logger.LogEventAsync(
                        "Данные spr_month не найдены",
                        nameof(LoadSprMonthDataAsync)
                    );
                }
                else
                {
                    await _logger.LogEventAsync(
                        "Данные spr_month успешно загружены",
                        nameof(LoadSprMonthDataAsync)
                    );
                }
            }
            ...
            {
                await _logger.LogErrorAsync(ex, "Ошибка загрузки данных spr_month");
            }
            finally
            {
                await this.InvokeAsync(() =>
        ...
        private async void PlanZagrVyazCheck_Load(object sender, EventArgs e)
        {
            ...
            Task loadSprMonthTask = LoadSprMonthDataAsync();
            await Task.WhenAll(loadSprMonthTask);
