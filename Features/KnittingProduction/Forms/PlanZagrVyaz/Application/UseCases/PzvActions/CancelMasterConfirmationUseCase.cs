...
{
    public sealed class CancelMasterConfirmationUseCase
    {
        private readonly PzvActionValidator _validator;
        private readonly IPzvBulkUpdateService _bulkUpdateService;

        public CancelMasterConfirmationUseCase(PzvActionValidator validator, IPzvBulkUpdateService bulkUpdateService)
        {
        ...
        public async Task<OperationResult> ExecuteAsync(PzvSelectionContext context, CancellationToken ct = default)
        {
            ...
            {
                if (_validator.CanCancelMasterConfirmation(row))
                {
                ...
                else
                {
                    _validator.ShowValidationMessage(
                        row,
                        _validator.ValidateCancelMasterConfirmation,
                        "Отмена подтверждения операции мастером");
            ...
            if (validIds.Count == 0)
                return OperationResult.Fail("Нет строк для отмены подтверждения мастером.");
