...
{
    public sealed class CancelWorkStopUseCase
    {
        private readonly PzvActionValidator _validator;
        private readonly IPzvBulkUpdateService _bulkUpdateService;

        public CancelWorkStopUseCase(PzvActionValidator validator, IPzvBulkUpdateService bulkUpdateService)
        {
        ...
        public async Task<OperationResult> ExecuteAsync(PzvSelectionContext context, CancellationToken ct = default)
        {
            ...
            {
                if (_validator.CanCancelStopWork(row))
                {
            ...
            if (validIds.Count == 0)
                return OperationResult.Fail("Нет строк для отмены завершения.");
