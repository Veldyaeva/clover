...
SELECT
  ...
  RTRIM(seb_vyaz_econom_one.nom_zadany) AS nom_zadany,
  RTRIM(MAX(seb_vyaz_econom_one.razm_ryad)) AS razm_ryad,
  seb_vyaz_econom_one.pach_min,
  seb_vyaz_econom_one.pach_max,
  seb_vyaz_econom_one.nom,
  RTRIM(MAX(seb_vyaz_econom_one.articul)) AS articul,
  RTRIM(MAX(seb_vyaz_econom_one.mod)) AS mod,
  MAX(seb_vyaz_econom_one.date_econom) AS date_econom,
  ISNULL(MAX(seb_vyaz_econom_one.last_otm_izm), 0) AS last_otm_izm,
  RTRIM(MAX(vib.grup)) AS grup,
