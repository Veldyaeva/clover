...
namespace SewingProduction.Features.Tabel.Services
{
    public class TabOtvlRDataService
    {
        ...
        public async Task<List<TabOtvlRModel>> GetByMgGrTabAsync(string mg, int gr, int tab)
        {
            const string query = @"
            SELECT
                ...
                orNew
            FROM tab_otvl_r
            WHERE mg = @mg AND gr = @gr AND tab = @tab
            ORDER BY dat, n_r, id";

            return await _dbService.GetListAsync<TabOtvlRModel>(query, new { mg, gr, tab });
        }
        ...
        public async Task SaveAsync(TabOtvlRModel model)
        {
            if (model == null) throw new ArgumentNullException(nameof(model));

            await _dbService.SaveEntityAsync("tab_otvl_r", "id", model);
        }
