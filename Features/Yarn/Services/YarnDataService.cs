...
namespace SewingProduction.Features.Yarn.Services
{
    internal sealed class YarnDataService
    {
        ...
        {
            const string sql = @"
                SELECT RTRIM(pp.nakl) AS nakl, RTRIM(pp.t_articul) AS t_articul,
                       RTRIM(pp.zvet) AS zvet, pp.seb_t_m
        ...
        {
            const string sql = @"
                SELECT DISTINCT RTRIM(pp.nakl) AS nakl, RTRIM(pp.t_articul) AS t_articul,
                       RTRIM(pp.zvet) AS zvet, pp.seb_t_m
                FROM dbo.prihod_pryz pp WITH (NOLOCK)
                INNER JOIN dbo.prihod_v pv WITH (NOLOCK) ON pp.kod_pr = pv.kod_pr
        ...
        public async Task<YarnRecalcResult> RecalculateCostAsync(string nakl, string kodArt)
        {
            ...
            return await connection.QuerySingleAsync<YarnRecalcResult>(
        ...
        {
            var sql = @"SELECT * FROM dbo.history_smena_seb_rekom WITH (NOLOCK) WHERE kodd = @Kodd"
                + (filterByKod ? " AND kod = @Kod" : "") + " ORDER BY data_izm";
        ...
        public Task<List<YarnArticulHistoryRow>> LoadArticulHistoryAsync(string kod)
        {
            const string sql = @"
                ...
                WHERE kod = @Kod ORDER BY data_smena DESC";
